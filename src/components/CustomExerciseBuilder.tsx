...
const CustomExerciseBuilder: React.FC = () => {
  ...
  const handleTypeSelection = (exerciseType: any) => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          instructions: JSON.parse(JSON.stringify(exerciseData.instructions)),
          target_joints: JSON.parse(JSON.stringify(exerciseData.targetJoints)),
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50">
        <header className="bg-white/80 backdrop-blur-sm border-b shadow-sm sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              <div>
                <h1 className="text-xl font-bold text-gray-900">Exercise Builder</h1>
                <p className="text-sm text-gray-500">Create personalized rehabilitation exercises</p>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <ExerciseTypeSelector 
            ...
            selectedType={selectedExerciseType?.id}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">
                  Building: {selectedExerciseType?.name}
                </h1>
                <p className="text-sm text-gray-500">Configure your custom exercise</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${currentStep === 'build' ? 'bg-blue-500' : 'bg-gray-300'}`} />
                <span className="text-sm">Build</span>
                <div className={`w-3 h-3 rounded-full ${currentStep === 'preview' ? 'bg-blue-500' : 'bg-gray-300'}`} />
                <span className="text-sm">Preview</span>
              </div>
              <Button 
                variant="outline" 
                onClick={() => setCurrentStep(currentStep === 'build' ? 'preview' : 'build')}
              >
                <Eye className="h-4 w-4 mr-2" />
                {currentStep === 'build' ? 'Preview' : 'Edit'}
              </Button>
              <Button onClick={saveExercise} className="bg-green-600 hover:bg-green-700">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Exercise Type Badge */}
        <div className="mb-6">
          <div className={`inline-flex items-center px-4 py-2 rounded-lg ${selectedExerciseType?.color || 'bg-gray-100'}`}>
            {selectedExerciseType?.icon && <selectedExerciseType.icon className="h-5 w-5 mr-2" />}
            <span className="font-medium">{selectedExerciseType?.name}</span>
        ...
        {currentStep === 'preview' ? (
          ...
          <Card className="border-2 border-green-200">
            ...
            <CardContent className="space-y-6 p-8">
              <div>
                <h2 className="text-2xl font-semibold mb-2">{exerciseData.title}</h2>
